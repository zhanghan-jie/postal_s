...
def scale_coords(img1_shape, coords, img0_shape):
    """
    Scale coordinates from the model input shape to the original image shape.

    Args:
        img1_shape (tuple): Shape of the input image after resizing and padding.
        coords (np.ndarray): Array of coordinates [x1, y1, x2, y2] for each bounding box.
        img0_shape (tuple): Original shape of the image before resizing and padding.
    ...
    # 将坐标缩放到原始图像大小
    coords[:, [0, 2]] -= pad[0]  # x坐标减去x方向的填充
    coords[:, [1, 3]] -= pad[1]  # y坐标减去y方向的填充
    coords[:, :4] /= gain  # 缩放比例
...
#     coords[:, :4] /= gain
#     coords[:, :4] = coords[:, :4].clip(0, img0_shape[1]), coords[:, 2:].clip(0, img0_shape[0])  # xyxy
#     return coords