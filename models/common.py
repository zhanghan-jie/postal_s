...
# import torch
# import torch.nn as nn
# from models.common import Conv  # 假设 Conv 类定义在一个单独的模块中
...
#         # 输入通道数
#         c_ = c2 // 2 if isinstance(c2, int) else c2  # hidden channels
#         self.cv1 = Conv(c1, c_, 1, 1)
#         self.cv2 = Conv(c1, c_, 1, 1)
#         self.cv3 = Conv(2 * c_, c2, 1)  # optional identity
#         self.m = nn.Sequential(*[Bottleneck(c_, c_, shortcut) for _ in range(n)])
#         # self.m = nn.Sequential(*[CrossConv(c_, c_, 3, 1, g, 1.0, shortcut) for _ in range(n)])  # crossconv
#
#     def forward(self, x):
